...
BEGIN_MESSAGE_MAP(CNMsgrClntDlg, CDialogEx)
	...
	ON_BN_CLICKED(ID_BUTTON_CONNECT3, &CNMsgrClntDlg::OnBnClickedButtonConnect3)
END_MESSAGE_MAP()
...
BOOL CNMsgrClntDlg::OnInitDialog()
{
	...
	m_Edit_ID.SetWindowTextA("ArmUp1_all");
	m_Edit_Delay.SetWindowText("30");

	m_bReceiveSkeletonMsg = true;

	SetWindowPos(NULL, 1280, 530, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
	...
	//slider
	m_sliderCtrl.SetRange(0, SLIDELEN);
...
unsigned __stdcall CNMsgrClntDlg::ReadThread(void* pArg)
{
	...
	StateVector tempVec[4];
	CString str;

	glBodyRenderer m_renderer0;
	glBodyRenderer m_renderer1;
	glBodyRenderer m_renderer2;
	glBodyRenderer m_renderer3;

	m_renderer0.InitializeRenderer("Motive", 40, 40);
	m_renderer0.WaitUntilThreadInit();
	m_renderer1.InitializeRenderer("Center Kinect", 690, 40);
	m_renderer1.WaitUntilThreadInit();
	m_renderer2.InitializeRenderer("Kinect Average", 40, 530);
	m_renderer2.WaitUntilThreadInit();
	m_renderer3.InitializeRenderer("Kinect Filtered", 690, 530);
	m_renderer3.WaitUntilThreadInit();
	...
	while(true){
		if(pDlg->isStoped != 1){
			...
			if(pDlg->m_reader.isOpened()){
				...
				int tSliPos = pDlg->m_sliderCtrl.GetPos();
				if(abs(pDlg->m_PrevSliderPos - tSliPos) > 2){
					pDlg->FrameCount = tSliPos * pDlg->m_reader.GetTotalFrame() / SLIDELEN;
					pDlg->m_reader.MoveFrame(pDlg->FrameCount);
				}

				if(!pDlg->m_reader.ReadAll(tempVec)){
					pDlg->m_reader.CloseFile();
					pDlg->m_sliderCtrl.SetPos(0);
					pDlg->PrintStatus("Gesture End");
				}else{

					str.Format("[%d] frame", pDlg->FrameCount);
					pDlg->PrintStatus(str);
					pDlg->m_sliderCtrl.SetPos( pDlg->FrameCount*SLIDELEN/ pDlg->m_reader.GetTotalFrame());

					m_renderer0.SetBodyInfo(&tempVec[0]);
					m_renderer1.SetBodyInfo(&tempVec[1]);
					m_renderer2.SetBodyInfo(&tempVec[2]);
					m_renderer3.SetBodyInfo(&tempVec[3]);

					m_renderer0.WaitUntilRenderCompelete();
					m_renderer1.WaitUntilRenderCompelete();
					m_renderer2.WaitUntilRenderCompelete();
					m_renderer3.WaitUntilRenderCompelete();

					pDlg->m_PrevSliderPos = pDlg->FrameCount*SLIDELEN/ pDlg->m_reader.GetTotalFrame();
				}

			}else{
				m_renderer0.SetBodyInfo(NULL);
				m_renderer1.SetBodyInfo(NULL);
				m_renderer2.SetBodyInfo(NULL);
				m_renderer3.SetBodyInfo(NULL);
...
void CNMsgrClntDlg::OnBnClickedButtonConnect3()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_reader.CloseFile();
}

void CNMsgrClntDlg::setMySlider(CSliderCtrl* m_pSliderCtrl){
