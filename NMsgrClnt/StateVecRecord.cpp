...
void StateVecRecord::OpenFile(char *Path, char *filename, char mode){
	...
	if(Path == NULL){	
		...
		sprintf(buf_all, "%s/%s.bin", DEFAULT_FILE_PATH, filename);
	}
	else{				
		...
		sprintf(buf_all, "%s/%s.bin", Path, filename);
	}
	...
	if(mode == 'r'){
		...
		m_all = fopen(buf_all, "rb");

		struct stat statbuf;
		fstat(fileno(m_all), &statbuf);

		m_Totalframe = statbuf.st_size / (sizeof(StateVector)*4);
...
int StateVecRecord::GetTotalFrame(){
	if(isOpen)
		return m_Totalframe;
	else return 0;
}

void StateVecRecord::MoveFrame(int TargetFrame){
	if(isOpen){
		fseek(m_all, 4 * TargetFrame * sizeof(StateVector), SEEK_SET);
	}
}